...
| ✅ **DNS** — relocated to `doggos.indigopod.au` (A record, unproxied) with `doggos.indigo.pod.haus` kept as a permanent redirect. | Remove `doggos.indigo` from `pod_haus_external_cnames`; add an A record to `numbat_relay_ipv4`, unproxied, in the `yiayia_pod_haus` shape. pod.haus is not a CDN zone; no cache contract. | `terraform/dns_pod_haus.tf` |
...
| ✅ **Repository** — `git.pod.haus/LogicWolfe/indigo-web-server`, archived there and on GitHub. | Move `indigo-web-server` to `git.pod.haus/LogicWolfe/indigo-web-server` as the record of the Railway era, archive the GitHub copy. The bucket is the live copy from then on; the repo is not updated. | Forgejo, GitHub |
| **Railway** — project *Indigo's Stuff* deleted ✅; the account is now empty. Closing it and deleting the 1Password item `railway-api-token` is Nathan's. | Delete *Indigo's Stuff*, close the account, delete the 1Password item `railway-api-token`. | Railway, 1Password |
