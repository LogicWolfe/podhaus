...
**Goal.** `doggos.indigo.pod.haus` is served from a versioned MinIO bucket on
...
| ✅ **Serving** — `(doggos_site)` snippet and `doggos.indigo.pod.haus:4444` block. | `doggos.indigo.pod.haus:4444` block: Cloudflare DNS TLS, `Cache-Control "no-cache"` (Blocs busts CSS/JS with query strings; images revalidate on ETag), styled `404.html` from the bucket with `replace_status`, `/` → `index.html`, `<dir>/` and extensionless → `<dir>/index.html`, everything else → `/doggos-indigo{path}`, `reverse_proxy minio:9000` turning `NoSuchKey` into a 404. The `sky_site` snippet minus its redirects and cache tags. | `caddy/Caddyfile` |
...
| ✅ **Monitoring** — Gatus entry added. | `doggos.indigo.pod.haus` external check in the `nathanbaxter.com` shape (200, `<<: *defaults`). | `gatus/conf/config.yaml` |
