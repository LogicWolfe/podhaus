...
| `forgejo/` | Local Git hosting on bilby. Web UI at `forge.pod.haus` uses Cloudflare Tunnel + Access; ordinary SSH at `git.pod.haus:22` uses kookaburra's Reserved-IP anchor → rathole → Forgejo embedded SSH. SQLite/config are local NVMe; repositories/LFS/attachments/archives are on Jump. `forgejo/provision/` is the desired state for managed accounts and `podhaus-managed:*` SSH keys; its reconciler runs on every deploy and must preserve unmanaged keys. See `docs/runbooks/forgejo.md`. |
