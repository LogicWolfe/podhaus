...
- **Every pinned LAN address has exactly ONE definition:
  `config/lan-addresses.json`.** Terraform reads it for DHCP reservations
  and split-horizon DNS; Ansible and bootstrap commands read it directly.
  `tools/render-lan-addresses.py` generates `komodo/sync/lan-addresses.toml`;
  ResourceSync publishes its `LAN_*` variables for stack environment mappings.
  The pre-commit check rejects generated values that differ from the catalog.
  **Never write a
  `10.0.0.x` host address into a compose file, `stack.toml`,
  `caddy/Caddyfile`, `gatus/conf/config.yaml`, an Alloy config, inventory,
  or a script** — edit the catalog, update its typed renderer when adding a
  field, regenerate the Komodo variables, and reference the shared value.
...
- **Never set `deploy = true` on a podhaus `stack.toml`.** Komodo's
  ...
  omit the field.)
- **Linked Repo hosts (kangaroo, numbat, fractal, voltaire, bandicoot, pinelake) use the
