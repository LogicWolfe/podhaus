...
| `logging/kookaburra/` | Alloy on kookaburra — ships container logs cross-tailnet to bilby's ClickStack at `100.87.76.25:4318` (bilby's tailnet IP; brittle to volume-regen drift — long-term fix is MagicDNS on kookaburra docker, parallels bilby's daemon.json setup). |
