...
| `pomerium-ssh-origin-bootstrap` | Generic target-side Pomerium SSH origin bootstrap. Reads the per-route token from stdin, installs the current rathole release, trusts the Pomerium CA, and reconciles either a hardened systemd service or the QTS persistent autorun adapter. The initial SSH transport stays outside the script. |
