...
| `ansible/roles/docs_sources/` | Stable read-only repository source slots for docs-server on Bilby, Fractal, and Voltaire. The recurring reconciler exposes each available user-owned checkout beneath `/opt/podhaus/docs-sources`; unavailable sources reveal a marker, making docs health red while other sources continue serving. |
