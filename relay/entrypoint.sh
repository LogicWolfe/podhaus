...
envsubst '${RATHOLE_TOKEN} ${RATHOLE_GIT_TOKEN} ${RATHOLE_NOISE_LOCAL_PRIVATE_KEY} ${RATHOLE_NOISE_REMOTE_PUBLIC_KEY} ${RATHOLE_ANCHOR_IPV4} ${RATHOLE_PUBLIC_TLS_TOKEN} ${RATHOLE_FORGEJO_SSH_TOKEN} ${RATHOLE_PROTECTED_HTTP_TOKEN} ${RATHOLE_BILBY_SSH_TOKEN} ${RATHOLE_VOLTAIRE_SSH_TOKEN} ${RATHOLE_KANGAROO_SSH_TOKEN} ${NUMBAT_APPLICATION_IPV4} ${NUMBAT_RELAY_IPV4}' \
  < "$TEMPLATE" > "$RENDERED"
